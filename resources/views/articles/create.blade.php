...
<div style="max-width: 600px; margin: 30px auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); background: #fff;">
    ...
    <form action="{{ route('articles.store', ['institution' => $institution->subdomain]) }}" method="POST">
        @csrf
        ...
        <div style="margin-bottom: 15px;">
            ...
            <textarea id="content" name="content" required 
                      style="width: 100%; height: 140px; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">{{ old('content') }}</textarea>
            @error('content')
                <p style="color:red; font-size: 13px;">{{ $message }}</p>
            @enderror
        </div>
